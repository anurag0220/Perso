...
http.listen(3000, function () {
    ...
    mongoClient.connect("mongodb+srv://anuragmishra02002:Shield@619@cluster0.vwlkwau.mongodb.net/", {
    ...
    if (request.session.user) {
        try {
            ...
            // Extract the specified file name from the form input, if provided
            const newName = request.fields.newName || fileName;
            ...
            fileStream.on('end', async function() {
                ...
                var existingFile = await database.collection("users").findOne({
                    "_id": ObjectId(request.session.user._id),
                    "uploaded.hash": fileHash
                });

                if (existingFile) {
                    // File with the same content already exists, handle duplication error
                    request.session.status = "error";
                    request.session.message = "File with the same content already exists.";
                ...
                var uploadedObj = {
                    ...
                    "name": newName, // Use the specified file name
                    "type": request.files.file.type,
                    "filePath": "",
                    "createdAt": new Date().getTime(),
                    "hash": fileHash // Save the hash for future reference
                };
                ...
                fileSystem.readFile(request.files.file.path, function (err, data) {
                    ...
                    fileSystem.writeFile(filePath, data, async function (err) {
                        ...
                        request.session.status = "success";
                        request.session.message = "Image has been uploaded successfully.";
